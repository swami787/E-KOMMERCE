...
=======
  ...
  // Filter out inappropriate products (like adult items)
  const filteredProducts = products.filter(
    (product) =>
      !product.name?.toLowerCase().includes("sex") &&
      !product.category?.toLowerCase().includes("adult")
  );
>>>>>>> 0828cff57d01b05c905c616d3ef548093836c954
...
>>>>>>> 236225a540b759a49ed59cba4fe7553da395a7ee
            ...
            // 🔍 Optional: Debug missing products
            if (!productData) {
...
=======
        {cartData.length === 0 && (
          <p className='text-white text-[20px] w-full text-center mt-[40px]'>
            Your cart is empty.
          </p>
        )}

        {cartData.map((item, index) => {
          const productData = filteredProducts.find((product) => product._id === item._id);
            ...
            <div key={index} className='w-[100%] h-[10%] border-t border-b'>
              <div className='w-[100%] h-[80%] flex items-start gap-6 bg-[#51808048] py-[10px] px-[20px] rounded-2xl relative'>
                ...
                <div className='flex items-start justify-center flex-col gap-[10px]'>
                  ...
                  <div className='flex items-center gap-[20px]'>
                    <p className='text-[20px] text-[#aaf4e7]'>{currency} {productData.price}</p>
                    <p className='w-[40px] h-[40px] text-[16px] text-white bg-[#518080b4] rounded-md mt-[5px] flex items-center justify-center border border-[#9ff9f9]'>{item.size}</p>
                  </div>
                </div>
